...
void XMLDocument::LoadFiles(const std::vector<std::string>& filePaths, const std::function<void(XMLDocument&)>& callback) {
    std::mutex mutex;
    std::condition_variable cv;
    std::queue<std::string, std::deque<std::string>> taskQueue(
        std::deque<std::string>(filePaths.begin(), filePaths.end()));
    std::vector<std::thread> threads;
    std::vector<std::string> errors; // To store error messages
    ...
    auto worker = [&]() {
        while (true) {
            ...
            if (doc.LoadFile(filePath.c_str()) == XML_SUCCESS) {
                ...
                errors.push_back("Failed to load: " + filePath);
            }
        }
    };

    // Spawn threads
    const size_t numThreads = std::min<size_t>(std::thread::hardware_concurrency(), filePaths.size());
    ...
    // Print errors (optional, can be handled differently)
